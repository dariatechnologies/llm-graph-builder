...
def process_documents(docs, question, messages, llm, model, prompt):
    start_time = time.time()
    formatted_docs, sources = format_documents(docs, model)
    rag_chain = get_rag_chain(llm=llm, system_template=prompt)
...
def QA_RAG(graph, model, question, document_names, session_id, mode, prompt):
    try:
        ...
        if docs:
            content, result, total_tokens = process_documents(
                docs, question, messages, llm, model, prompt
            )
        else:
